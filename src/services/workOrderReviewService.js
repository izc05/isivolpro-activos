...
export async function loadWorkOrderFinalReview(tenantId, workOrderId) {
  const [checklistRes, visitsRes, materialsRes, photosRes, reportsRes] = await Promise.all([
  ...
  const firstError = [checklistRes.error, visitsRes.error, materialsRes.error, photosRes.error, reportsRes.error].find(Boolean);
...
export async function validateWorkOrderAsAdmin(workOrder, reviewItems, notes = '') {
  ...
  return updateWorkOrderLifecycleStatus(workOrder, 'VALIDADA', { adminNotes: notes });
}
