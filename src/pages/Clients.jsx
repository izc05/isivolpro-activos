...
import { createTenantAsOwner, updateTenant } from '../services/tenantService';
import { useAuth } from '../hooks/useAuth';

export default function Clients() {
  const { tenants, refreshTenants, setActiveTenantId } = useTenant();
  ...
  return (
    <>
      <PageHeader title="Clientes" subtitle="Modelo SaaS multi-cliente: cada cliente vive separado por tenant_id dentro de la misma plataforma." action={<button className="primary-button" onClick={startCreate}>Nuevo cliente</button>} />
      <DataTable columns={[
        { key: 'nombre', label: 'Cliente' },
        { key: 'cif', label: 'CIF' },
        { key: 'email', label: 'Email' },
        { key: 'plan', label: 'Plan', render: (row) => <span className="badge">{row.plan || 'starter'}</span> },
        { key: 'billing_status', label: 'Suscripcion', render: (row) => <span className={row.billing_status === 'active' ? 'badge ok' : 'badge warn'}>{row.billing_status || 'trial'}</span> },
        { key: 'estado', label: 'Estado', render: (row) => <span className="badge ok">{row.estado}</span> },
        { key: 'actions', label: 'Acciones', render: (row) => <button className="secondary-button" onClick={() => startEdit(row)}>Editar</button> }
      ]} rows={tenants} />
          ...
          {editing && (
            <>
              ...
              <div className="card subtle-card">
                ...
                <p className="muted">Estos datos preparan el modelo de pago por cliente. Solo el propietario de la plataforma deberia cambiarlos.</p>
                <div className="grid two">
                  <FormField label="Plan">
                    <select value={form.plan} onChange={(event) => updateField('plan', event.target.value)} disabled={!isSuperAdmin}>
                      <option value="starter">Starter</option>
                      <option value="pro">Pro</option>
                      <option value="empresa">Empresa</option>
                    </select>
                  </FormField>
                  <FormField label="Estado suscripcion">
                    <select value={form.billing_status} onChange={(event) => updateField('billing_status', event.target.value)} disabled={!isSuperAdmin}>
