...
import { OFFICIAL_WORK_ORDER_TYPES, priorityLabel, workOrderTypeLabel } from '../utils/workOrderLifecycle';
...
export default function Incidents() {
  const { rows, activeTenantId, refresh } = useTenantRows('incidencias', '*, instalaciones(nombre), ubicaciones(nombre), activos(nombre)', { order: 'fecha_apertura' });
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) => {
      const searchable = `${row.titulo || ''} ${row.descripcion || ''} ${row.instalaciones?.nombre || ''} ${row.ubicaciones?.nombre || ''} ${row.activos?.nombre || ''}`.toLowerCase();
  ...
  return (
    <>
      ...
      <CollapsibleSection title="Resumen incidencias" subtitle="Avisos abiertos, en revisión y convertidos en OT" icon={ClipboardCheck} badge={`${stats.abiertas} abiertas`} defaultOpen>
      ...
      <CollapsibleSection title="Filtros" subtitle="Busca por aviso, instalación, ubicación o activo" icon={Filter} badge={`${filteredRows.length}/${rows.length}`} defaultOpen>
        <div className="user-filter-grid">
          <label><span>Buscar</span><input value={filters.search} onChange={(event) => setFilters((current) => ({ ...current, search: event.target.value }))} placeholder="Título, descripción, instalación o activo" /></label>
